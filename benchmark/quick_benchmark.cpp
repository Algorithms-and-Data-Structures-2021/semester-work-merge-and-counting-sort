...
static constexpr auto kDatasetPath = string_view{PROJECT_DATASET_DIR_QUICK_SORT};
...
int main() {
  ...
  const auto output_path = string(kProjectPath) + "/benchmark/results/qsortResults.csv";
    ...
    for (string folder : folders) {   // Проходим по всем 10 папкам
      for (int i = 1; i < 11; i++) {  // Запускаем замерку времени 10 раз
        ...
        if (input_file) {
          ...
          itis::qsortRecursive(dataKeeper, n);
